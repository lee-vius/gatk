 ...
 *     <ul>
 *         <li>CIGAR having clipping neighboring non-alignment blocks (alignments will have a tag "NG:i:1")</li>
 *         <li>CIGAR having an insertion/deletion operation neighboring a deletion/insertion operation
 *             (alignments will have a tag "NS:i:1")</li>
 *     </ul>
 * </p>
 *
 * <p>
...
public class ExtractAlignmentsWithStrangeCigarSpark extends GATKSparkTool {
    ...
    public static final String NEIGHBOR_INSDEL_OP = "NG";
    public static final String NEIGHBOR_CLIPGAP_OP = "NS";
    ...
    protected void runTool( final JavaSparkContext ctx ) {
        ...
        final ReadFilter compositeFilter = catchDelNeighboringIns.or(catchNonAlnNeighboringClip);

        final JavaRDD<GATKRead> strangeCigaredReads =
                getReads()
                        .filter(r -> !r.isUnmapped())
                        .filter(compositeFilter::test)
                        .map(read -> {
                            GATKRead copy = read.copy();
                            if (catchDelNeighboringIns.test(read)) {
                                copy.setAttribute(NEIGHBOR_INSDEL_OP, 1);
                            } else if (catchNonAlnNeighboringClip.test(read)) {
                                copy.setAttribute(NEIGHBOR_CLIPGAP_OP, 1);
                            }
                            return copy;
                        });
        writeReads(ctx, output, strangeCigaredReads, getHeaderForReads(), false);
    }
